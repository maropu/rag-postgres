...
def _get_conda_bin_executable(executable_name):
    ...
    ``CONDA_HOME`` environment variable. If
    ``CONDA_HOME`` is unspecified, this method simply returns the passed-in
    executable name.
    """
...
def _get_or_create_conda_env(conda_env_path, env_name_prefix):
    ...
    needs_to_create = project_env_name not in env_names
    if needs_to_create:
        if conda_env_path:
    ...
    return project_env_name, needs_to_create
...
if __name__ == "__main__":
    ...
    conda_env_name, is_created = _get_or_create_conda_env(conda_env_path, args.env_id)
